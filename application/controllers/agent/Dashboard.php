...
class Dashboard extends BaseController
{
    ...
    public function index()
    {
        ...
        $this->agentView("agent/dashboard", $this->global, NULL , NULL);
    }
    ...
    public function inquiry($id = NULL)
    {
        ...
        $where = array();
        $where['id'] =$id;
        $where['assign_to'] =$this->session->userdata('userId');
        $results = $this->contact_agent_model->findDynamic($where);
        /*echo "<pre>";
print_r($results );
echo "</pre>";*/
        if(!empty($results))
        {
             $data['edit_data']  = $results[0];
