...
class Agency extends BaseController
{
      ...
      function deleteagent()
    {
        ...
        $result = $this->staff_model->delete($delId); 
...
// Member list
    public function ajax_Staff_list()
    {
        ...
        foreach ($list as $currentObj) {
            ...
            $filename = (isset($currentObj->StaffTumbnail) && $currentObj->StaffTumbnail !=='') ? ($currentObj->StaffTumbnail) : ("");
            $temp_date = $currentObj->date_at;
            $selected = ($currentObj->status == 0)?" selected ":"";
            $btn = '<select class="statusBtn" name="statusBtn" data-id="'.$currentObj->id.'">';
            $btn .= '<option value="1"  >Active</option>';
     ...
     public function add_new_staff()
    {
        ...
        $this->form_validation->set_rules('StaffPrimaryNumber','Primary','required');
        ...
        else
        {
               ...
               // staff_query
                $StaffEmail = strtolower($form_data['StaffEmail']) ; 
                $sql_where  = " StaffEmail = '$StaffEmail' AND status='1'";
                ...
                if(isset($form_data['StaffID']) && $form_data['StaffID'] !=='')
                {
                    ...
                    $sql_where .=" AND id !='$StaffID'";
                }


                 $stafexist = $this->staff_model->staff_query($sql_where);
                 ...
                 if(empty($stafexist))
                 {
                    ...
                    $insertData['StaffEmail']       = $StaffEmail;
                    $insertData['StaffPassword']    = md5($form_data['StaffPassword']);
                    $insertData['password_decripted']= $form_data['StaffPassword'];
                    ...
                    $insertData['StaffPrimaryNumber'] = $form_data['StaffPrimaryNumber'];/*
                 ...
                 }else
                 {
                    $response = array(
                            'status' => 'error',
                            'message' => "<h3>Staff Member Already Exist.</h3>"
                        );
                 }
             }  
          $this->output
