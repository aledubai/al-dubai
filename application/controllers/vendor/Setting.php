...
class Setting extends BaseController
{
      ...
      public function profile()
    {
        ...
        $data['profile_data'] = '';
        if(!empty($setting_profile))
        {
            $data['profile_data']       =  $setting_profile[0] ;    
        }
    ...
    public function add_setting_profile()
    {
        ...
        {
                $strlower = strtolower($form_data['ProfileName']);


                $my_str = ltrim($strlower);
                $slug_url  = str_replace(" ","-",$my_str);
                $where                      = array();
                $where['table']             = 'setting_profile';
                $where['slug_url']          = $slug_url;
                $where['user_id !=']         =  $this->session->userdata ( 'userId' );
                $slug_url_exits            = $this->settingprofile_model->findDynamic($where);
                if(!empty($slug_url_exits))
                {
                     $this->session->set_flashdata('error', 'Slug Url Already Exist');
                }else
                {
                    $insertData = array();
                    ...
                    // Fields Details
                     ...
                     if(isset($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
                        ...
                        $store          ="uploads/vendor/".$f_newfile;
                    ...
                    $insertData['ServiceArea']          = $form_data['ServiceArea'];;
