...
print_r($this->db->last_query());
       ...
       // Get  List
        ...
        public function getSubLocationList(){
            ...
            $query = $this->db->select('*')
                    ->from('ae_rel_sublocation_list')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
                      $subLocation[$v->id] = $v->name;
                   } 
            return $subLocation;
        }
        ...
        function getDeveloperList(){
           ...
           /* $developer[1] = " Azizi Developments";
            ...
            $query = $this->db->select('*')
                    ->from('ae_rel_developer_list')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
                      $developer[$v->id] = $v->name;
                   } 
            return $developer;
        }
        ...
        public function listFurnissing(){
            ...
            // $furnissing[2] = "UnFurnished";
             ...
             $query = $this->db->select('*')
                    ->from('ae_rel_list_furnissing')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
                      $furnissing[$v->id] = $v->name;
                   } 
            return $furnissing;
        }
        ...
        public function listEmailTempType(){
            ...
            // $temp[2] = "Email";
            $query = $this->db->select('*')
                    ->from('ae_rel_list_email_temp_type')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
        ...
        public function listTypeOfContact(){
            ...
            // $arr[2] = "Company";
             $query = $this->db->select('*')
                    ->from('ae_rel_list_type_of_contact')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
        ...
        public function listStateIndia(){
            ...
            $query = $this->db->select('*')
                    ->from('ae_rel_list_state_india')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
                      $listStateIndia[$v->id] = $v->name;
                   }
        ...
        public function listPropertyStatus(){
            ...
            // $list[4] = "Review";
             $query = $this->db->select('*')
                    ->from('ae_rel_list_property_status')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
        ...
        public function listFeatureStatus(){
            ...
            $query = $this->db->select('*')
                    ->from('ae_rel_list_feature_status')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
        ...
        public function reminderEmail(){
            ...
            $query = $this->db->select('*')
                    ->from('ae_rel_reminder_email')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
                      $reminder[$v->id] = $v->name;
                   }
            return $reminder;
        ...
        public function RecreationAndFamily(){
            ...
            // $list[5] = "Cafeteria or Canteen";
             $query = $this->db->select('*')
                    ->from(' ae_rel_recreation_and_family')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
        ...
        // function RecreationAndFamily
        public function RecreationAndFamilyIcon(){
            $list = array();
            $list[1] = "barbeque-area.svg";//Barbeque Area
            $list[2] = "day_care_center.svg";//Day Care Center
            $list[3] = "kids_play_area.svg";//Kids Play Area
            $list[4] = "lawn_or_garden.svg";//Lawn or Garden
            $list[5] = "cafeteria_or_canteen.svg";//Cafeteria or Canteen
            return $list;
        }

        // function HealthAndFitness
        public function HealthAndFitnessIcon(){
            $list = array();
            $list[1] = "first_aid_medical_center.svg";//First Aid Medical Center
            $list[2] = "gym_or_health_club.svg";//Gym or Health Club
            $list[3] = "jacuzzi.svg";//Jacuzzi
            $list[4] = "sauna.svg";//Sauna
            $list[5] = "steam_room.svg";//Steam Room
            $list[6] = "swimming_pool.svg";//Swimming Pool
            $list[7] = "facilities_for_disabled.svg";//Facilities for Disabled
            return $list;
        }
        // function HealthAndFitness icon
        public function HealthAndFitness(){
            ...
            // $list[7] = "Facilities for Disabled";
              $query = $this->db->select('*')
                    ->from('ae_rel_health_and_fitness')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
        ...
        public function LaundryAndKitchen(){
            ...
            // $list[3] = "Shared Kitchen";
              $query = $this->db->select('*')
                    ->from('ae_rel_laundry_and_kitchen')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
         ...
         // function LaundryAndKitchen
        public function LaundryAndKitchenIcon(){
            $list = array();
            $list[1] = "laundry_room.svg";//Laundry Room
            $list[2] = "laundry_facility.svg";//Laundry Facility
            $list[3] = "shared_kitchen.svg";//Shared Kitchen
            return $list;
        }
        ...
        public function BuildingList(){
         ...
         // function LaundryAndKitchen
        public function BuildingListIcon(){
            $list = array();
            $list[1] = "balcany-or-tarec.svg";//Balcony or Terrace
            $list[2] = "lobby_in_building.svg";//Lobby in Building
            $list[3] = "service_elevator.svg";//Service Elevators
            $list[4] = "prayer-room.svg";//Prayer Room
            $list[5] = "reception_waiting_room.svg";//Reception/Waiting Room
            return $list;
        }
        ...
        public function BusinessAndSecurity(){
            ...
            // $list[4] = "CCTV Security";
             $query = $this->db->select('*')
                    ->from('ae_rel_business_and_security')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
        ...
        // function BusinessAndSecurityIcon
        public function BusinessAndSecurityIcon(){
            $list = array();
            $list[1] = "business_center.svg";//Business Center
            $list[2] = "conference_room.svg";//Conference Room
            $list[3] = "security_staff.svg";//Security Staff
            $list[4] = "cctv-security.svg";//CCTV Security
            return $list;
        }
        ...
        public function MiscellaneousList(){
            ...
            // $list[4] = "24 Hours Concierge";
             $query = $this->db->select('*')
                    ->from('ae_rel_miscellaneous_list')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
        ...
        // function MiscellaneousList icon
        public function MiscellaneousListIcon(){
            $list = array();
            $list[1] = "freehold.svg";
            $list[2] = "atm_facility.svg";
            $list[3] = "maids_room.svg";
            $list[4] = "24_ours_concierge.svg";//24 Hours Concierge
            return $list;
        }
        ...
        // function TechnologyList
        public function TechnologyListIcon(){
            $list = array();
            $list[1] = "broadband_Internet.svg";//Broadband Internet
            $list[2] = "satellite_cable_tv.svg";//Satellite/Cable TV
            $list[3] = "intercom.svg";//Intercom
            return $list;
        }
        ...
        // function FeaturesList
        public function FeaturesListIcon(){
            $list = array();
            $list[1] = "double_glazed_windows.svg";//Double Glazed Windows
            $list[2] = "centrally_air_conditioned.svg";//Centrally Air-Conditioned
            $list[3] = "central_heating.svg";//Central Heating
            $list[4] = "electricity_backup.svg";//Electricity Backup
            $list[5] = "furnished.svg";//Furnished
            $list[6] = "storage_areas.svg";//Storage Areas
            $list[7] = "study_room.svg";//Study Room
            return $list;
        }
        ...
        public function CleaningAndMaintenanceList(){
            ...
            // $list[3] = "Cleaning Services";
             ...
             $query = $this->db->select('*')
                    ->from('ae_rel_cleaning_and_maintenance_list')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
        ...
        // function CleaningAndMaintenanceList
        public function CleaningAndMaintenanceListIcon(){
            $list = array();
            $list[1] = "waste_disposal.svg";//Waste Disposal
            $list[2] = "maintenance_staff.svg";//Maintenance Staff
            $list[3] = "cleaning_services.svg";//Cleaning Services
            return $list;
        }
 ...
 // function listPropertyStatus
        public function listContactStatus(){
            ...
            $query = $this->db->select('*')
                    ->from('ae_rel_list_contact_status')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
        ...
        public function ActiveInactiveList(){
            ...
            $query = $this->db->select('*')
                    ->from('ae_rel_active_inactive_list')
                    ->where('status', '1')
                    ->get();
            $result = $query->result(); 
            foreach ($result as $v) {
