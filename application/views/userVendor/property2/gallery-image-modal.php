			...
			<div class="row">
				<div id="preview">
					 <?php 
					 	...
					 	if( !empty($PropertyImagesData[0]) && !empty($PropertyImagesData[0]))
					 	{ 
							$PropertyImagesData = $PropertyImagesData[0];
							...
							foreach ($PropertyImagesDataVal as $k =>$v ){ 
								if(isset($k) &&  $k !=='')
								{
									...
									<div class="col-sm-4 mt-4 alert alert-secondary box-shadow-rc alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
										...
										<input type="text" readonly="" style="width:100%;" name="imageName[]" value="<?=$PropertyImagesDataVal[$k]?>" id="imageName">
									</div>
									<?php     
								}
							} 
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			{  
				...
				$.ajax({  
				url:"<?php echo base_url(); ?>userVendor/ajax_upload_file/multipleImageStore",   
				method:"POST",  
				data:form_data,  
				contentType: false,  
				cache: false,  
 ...
 function readImage(file,index)
 {
	...
	reader.onload = function(_file) {
		...
		$('#preview').append('<div class="col-sm-4 mt-4 alert alert-secondary box-shadow-rc alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button> '+
				'<img style="height:160px;width: 100%;" src="' + this.src + '" class="thumb">' +'<input type="text" readonly name="imageName[]"  id="imageName'+index+'" style="width:100%;"></div>');
		};
